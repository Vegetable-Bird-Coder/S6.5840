...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if len(rf.log)-1 < args.PrevLogIndex { // log is too short
		if args.Entries == nil {
			fmt.Printf("Raft %d term %d refuses Heartbeat message from Raft %d term %d because log is too short\n", rf.me, currentTerm, args.LeaderId, args.Term)
		} else {
			fmt.Printf("Raft %d term %d refuses AppendEntries message from Raft %d term %d because log is too short\n", rf.me, currentTerm, args.LeaderId, args.Term)
		}
		reply.XTerm = -1
		...
		if args.Entries == nil {
			fmt.Printf("Raft %d term %d refuses Heartbeat message from Raft %d term %d because term confliction\n", rf.me, currentTerm, args.LeaderId, args.Term)
		} else {
			fmt.Printf("Raft %d term %d refuses AppendEntries message from Raft %d term %d because term confliction\n", rf.me, currentTerm, args.LeaderId, args.Term)
		}

		reply.XTerm = rf.log[args.PrevLogIndex].Term
		reply.XIndex = rf.findTermFirstIndex(reply.Term)
		reply.Success = false
	} else {
		if args.Entries == nil {
