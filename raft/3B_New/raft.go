...
func (rf *Raft) logConflict(args *AppendEntriesArgs, reply *AppendEntriesReply) bool {
	...
	if args.PrevLogIndex > lastLogEntry.Index {
		reply.XTerm, reply.XIndex, reply.Success = -1, lastLogEntry.Index, false
...
func (rf *Raft) applier() {
	for !rf.killed() {
		...
		entries := rf.log[rf.lastApplied+1-firstLogEntryIndex : commitIndex+1-firstLogEntryIndex]
		rf.mu.Unlock()
		...
		for _, entry := range entries {
			...
			fmt.Printf("Raft %d applied log index %d command %v\n", rf.me, entry.Index, entry.Command)
		}
...
func (rf *Raft) handleAppendEntriesResponse(peer int, request *AppendEntriesArgs, response *AppendEntriesReply) {
	...
	if response.XTerm != -1 {
		for i := len(rf.log) - 1; i >= 0; i-- {
			if rf.log[i].Term == response.XTerm {
				rf.nextIndex[peer] = rf.log[i].Index + 1
				break
			} else if rf.log[i].Term < response.XTerm {
				rf.nextIndex[peer] = response.XIndex // overwrite all entries with XTerm
				break
			}
		}
	} else {
